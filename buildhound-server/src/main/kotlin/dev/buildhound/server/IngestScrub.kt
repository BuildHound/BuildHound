 ...
 * before they run (plan 076). But since 076 the scrub runs at ingest *before*
 * [dev.buildhound.commons.payload.PayloadCapper]'s count caps, so the **number** of strings
 * scrubbed is unbounded: a single authenticated POST (32 MB gzip → 64 MB inflated) can carry
 * thousands of pathological ~8 KiB strings in per-task `executionReasons`/test messages and pin one
 * Netty worker for many minutes — a multi-tenant CPU DoS that rate limiting cannot stop (one
 * request suffices).
 *
 ...
 * short honest reasons scrub in well under a second, far below [BUDGET_MS]. Only an
 * all-pathological payload reaches the deadline, and it stops within one 8 KiB-clamped string (~2 s
 * worst) of it, so total scrub CPU is bounded to roughly [BUDGET_MS] + one clamped-string
 * worst-case (~5 s).
 *
...
object IngestScrub {
    /**
     * Redaction placed on every free-text field scrubbed after the budget is spent (a visible
     * signal).
     */
     ...
     * — one instance drives one single-threaded [PayloadScrubber.scrub] pass, which is exactly how
     * it is used (a fresh instance per ingest request).
     */
