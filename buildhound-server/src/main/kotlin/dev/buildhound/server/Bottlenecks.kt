...
@Serializable
data class KpiDelta(val current: Double? = null, val prior: Double? = null, val deltaPct: Double? = null)
...
/**
 * "What got worse this week" (plan 032, spec §6): a this-period-vs-prior-equal-period comparison over
 * four families, plus headline KPIs. Coarse and always-available (not plan 025's median+MAD baseline);
 * [budgetBreaches]/[trendRegressions] are populated only when a verdict store is wired (null → the UI
 * omits the card). [cacheDataAvailable] is false until `cacheable` is populated (plan 016).
 */
@Serializable
 ...
 * occurrences to enter the regressed ranking, and new/vanished groups are flagged rather than divided.
 */
object BottleneckCalculator {
    ...
    fun compute(
        ...
        val regressed = (current.keys + prior.keys).mapNotNull { key ->
            val c = current[key]
            val p = prior[key]
            when {
                c != null && c.count >= MIN_SAMPLES && p != null && p.count >= MIN_SAMPLES -> {
                    val delta = c.avgMs - p.avgMs
                    BottleneckRow(
                        key = key, module = c.module, currentMs = c.avgMs, priorMs = p.avgMs, deltaMs = delta,
                        deltaPct = if (p.avgMs > 0) roundTo6(delta.toDouble() / p.avgMs) else null, count = c.count,
                    )
                }
                c != null && c.count >= MIN_SAMPLES && p == null ->
                    BottleneckRow(key = key, module = c.module, currentMs = c.avgMs, deltaMs = c.avgMs, isNew = true, count = c.count)
                // "Vanished" means it did not execute at all this window (c == null). A group that ran
                // below the min-sample threshold is dropped, not zeroed-and-flagged-gone (honesty).
                c == null && p != null && p.count >= MIN_SAMPLES ->
                    BottleneckRow(key = key, module = p.module, currentMs = 0, priorMs = p.avgMs, deltaMs = -p.avgMs, isVanished = true, count = 0)
                else -> null
            }
        ...
        val slowest = currentTasks.groupBy { it.type ?: it.name }
            .map { (key, group) ->
                BottleneckRow(
                    key = key, module = group.map { it.module }.distinct().singleOrNull(),
                    currentMs = group.sumOf { it.durationMs }, count = group.size,
                )
            }
            .sortedWith(compareByDescending<BottleneckRow> { it.currentMs }.thenBy { it.key })
            .take(TOP_N)

        val negativeAvoidance = RollupCalculator.negativeAvoidance(currentTasks)
            .map { BottleneckRow(key = it.key, currentMs = it.totalExcessMs, count = it.count) }

        val misses = currentTasks.filter { it.cacheable == true && it.outcome == "EXECUTED" }
        val cacheMiss = misses.groupBy { it.type ?: it.name }
            .map { (key, group) ->
                BottleneckRow(
                    key = key, module = group.map { it.module }.distinct().singleOrNull(),
                    currentMs = group.sumOf { it.durationMs }, count = group.size,
    ...
    private fun executedGroups(rows: List<TaskRow>): Map<String, Group> =
        rows.filter { it.outcome == "EXECUTED" }.groupBy { it.type ?: it.name }.mapValues { (_, group) ->
            Group(
    ...
    private fun kpi(currentRaw: Double?, priorRaw: Double?): KpiDelta {
        ...
        return KpiDelta(current, prior, if (current != null && prior != null && prior != 0.0) roundTo6((current - prior) / prior) else null)
    }

    private fun successRate(builds: List<BuildKpiRow>): Double? =
        if (builds.isEmpty()) null else builds.count { it.outcome == "SUCCESS" }.toDouble() / builds.size
    ...
    // multiset of hit rates but feed rows in different orders, so an unsorted sum could differ in the
    // last bit and break byte-for-byte parity. (successRate/avgDuration sum integers → already exact.)
...
object ToolchainCalculator {
    ...
    fun dimension(samples: List<ToolchainSample>): ToolchainDimension {
        ...
        // comparator is never mislabeled "behind" on a bare string difference (e.g. "8.0" vs "8.0.0").
            ...
            val cmp = when {
                ...
                y == null -> 1
                    ...
                    when {
                        ...
                        xi != null -> 1 // a numeric segment sorts above a non-numeric one ("9" > "beta")
                        yi != null -> -1
                        else -> x.compareTo(y)
                    }
                }
