...
data class CompareBuildRef(
    ...
    val sha: String? = null,
    /**
     * The payload's root-project name (plan 079); powers the dashboard's same-project comparison
     * guard. Additive.
     */
...
/**
 * One differing input between the two builds, ranked by [coverage] — the share of the misses in B
 * that this input could explain. Build-scoped inputs cover all misses (1.0); task-scoped inputs
...
/**
 * Comparison of two builds A and B (plan 022, spec §5). A is treated as the "fast/cached" build, B
 * as the one whose cache misses we explain: [missesToExplain] are tasks that executed in B but were
 * avoided in A. [diffs] rank the differing inputs by how much of those misses each could account
 * for. All input values are salted hashes — no plaintext.
 */
@Serializable
...
object BuildComparator {
    ...
    private fun ref(p: BuildPayload) =
        CompareBuildRef(
            buildId = p.buildId,
    ...
    private fun missesToExplain(a: BuildPayload, b: BuildPayload): List<String> {
        val avoidedInA =
            a.tasks
                .filter {
                    it.outcome == TaskOutcome.FROM_CACHE || it.outcome == TaskOutcome.UP_TO_DATE
                }
                .map { it.path }
                .toSet()
        return b.tasks
            .filter {
                (it.outcome == TaskOutcome.EXECUTED || it.outcome == TaskOutcome.FAILED) &&
                    it.path in avoidedInA
            }
            .map { it.path }
            .distinct()
            .sorted()
    ...
    private fun buildLevelDiffs(a: BuildPayload, b: BuildPayload): List<CompareDiff> {
        ...
        return (fa.keys + fb.keys)
            .sorted()
            .filter { fa[it] != fb[it] }
            .map { key ->
                CompareDiff(key, "BUILD", fa[key], fb[key], coverage = 1.0, note = NOTES[key])
            }
    }
    ...
    private fun fieldDiffs(a: BuildPayload, b: BuildPayload): List<CompareDiff> {
        ...
        return fields
            ...
            .map { (key, v) ->
                CompareDiff(key, "FIELD", v.first, v.second, coverage = 1.0, note = NOTES[key])
            }
    }
    ...
    private fun taskLevelDiffs(
        ...
        return perKeyDiffCount.map { (key, count) ->
            CompareDiff(
                key,
                "TASK",
                differingTaskCount = count,
                coverage = count.toDouble() / misses.size,
                note = NOTES[key],
            )
        }
    }
     ...
     * volatile built-in key (e.g. `jdk.home`) that doesn't match its own
     * credential/timestamp/run-id name patterns — reuse, not extraction: this shipped, tested
     * plan-022 catalog stays put, and plan 068 only reads it through this accessor.
     */
    ...
    private val NOTES: Map<String, String> =
        mapOf(
            "jdk.home" to
                "JDK install path differs — Gradle keys the cache on the JDK major only, so a " +
                    "different home (even same major) can miss (android-cache-fix JdkImageWorkaround).",
            "jdk.vendor" to "JDK vendor differs.",
            "jdk.version" to "Full JDK runtime version differs (Gradle keys on the major only).",
            "file.encoding" to
                "Default file encoding differs — affects compilation and text I/O caching.",
