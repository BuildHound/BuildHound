...
-- left(…, 256) mirrors the server-side MAX_HOT_STRING_CHARS clamp (plan 077's
-- MAX_PROJECT_KEY_CHARS, absorbed by plan 078) so one pre-existing oversized key from any tenant
-- cannot fail the CREATE INDEX below and brick the whole deployment on upgrade. left() counts
-- codepoints where the server clamp counts UTF-16 units — accepted, cosmetic, both far below the
-- btree tuple limit.
