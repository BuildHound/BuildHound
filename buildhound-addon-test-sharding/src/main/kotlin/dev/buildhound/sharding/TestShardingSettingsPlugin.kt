...
class TestShardingSettingsPlugin : Plugin<Settings> {
    ...
    override fun apply(settings: Settings) {
        runCatching {
            ...
            val service = settings.gradle.sharedServices.registerIfAbsent("buildhoundTestShardPlan", ShardPlanService::class.java) { spec ->
            ...
            // Wire a filter on every Test task (IP-safe per-project hook). The service reference is
            // declared via usesService so it is a proper CC input; the doFirst reads the task from its
            // own argument (never a captured Task) and applies the shard filter at execution time.
            val isLastShard = index == total
