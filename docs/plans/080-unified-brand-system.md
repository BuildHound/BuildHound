...
- Create a new **Signal H** identity: a compact H-shaped tracking path with a detached signal
  point, delivered as full-color, monochrome, small-size, wordmark, favicon, and app-icon
  assets.
...
- Deliver a complete six-state status-icon set and exercise the system with representative
...
Copper remains exclusively brand-owned. Functional states use green (success/cache), blue
(information/up-to-date), red (failure), gold (warning/flaky), cyan (running), and neutral
(skipped/cache miss); every state has explicit light/dark foreground and soft-surface aliases.
...
- The Signal H may read as a generic letter rather than tracking; validate at 16/24/44 px,
...
- Full-color, mono, small, wordmark, favicon/app-icon, six status icons, fonts, and CSS/JSON
