...
*Machine-specs residual (plan 104):* `environment.machine.diskTotalMb` joins that list, and is a **stronger** signal than the `cores`/`ramMb` it sits beside — those take a handful of realistic values, whereas a filesystem capacity (e.g. `486400`) is high-entropy and stable per machine, so a build series can be tied to one workstation by it alone. `diskFreeMb` is weaker (it moves every build) but is a within-machine signal in the same family. Both are collected deliberately — disk pressure is a real build-failure cause and capacity is the number that explains it — and neither is a path or a device name (see §4). Recording them here so `pseudonymize=strict` has a complete coarsening target list when it lands: capacity should bucket (e.g. to the nearest 100 GB) rather than suppress, since the diagnostic value survives bucketing and the identifying value does not. `diskMedia` is a five-value enum and needs no treatment.
...
*As built (plan 107):* the **Build detail** page gains a **Machine** section — hardware chips (os/arch, cores, memory, disk media + free/total, max workers, CI runner attributes) plus a resource-usage chip row (daemon CPU utilization over the execution window, window length, system CPU load, load average, system memory used/total) carrying the artifact's honesty caveat, that the window is the Gradle daemon's execution phase and so excludes configuration and every forked JVM — and a **CPU** column on the Process snapshot table (`processes[].cpuTimeMs`). The standalone HTML artifact has carried all of this since plan 104; the dashboard reads it straight from the full `BuildPayload` that `GET /v1/builds/{buildId}` already returns, so again **no new endpoint, projection, schema field, or migration** was added. The hardware chips read `environment` fields that have existed since schema v1, so a payload predating plan 104 renders the hardware row and no resource-usage row; only a payload carrying neither machine-relevant `environment` data nor `resourceUsage` hides the section outright. Cross-build *aggregation* of these fields (a machine/runner cohort view, a resource-usage trend) is a deliberate follow-up — it would need an ingest-time projection into a hot table, the same distinction drawn for warnings above.
