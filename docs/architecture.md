...
12. **Task-graph-derived data is captured in `settings.gradle.taskGraph.whenReady`.**
    ...
    **Composite-build caveat (plan 044):** the "build-service parameter finalized after
    ...
    data a **finalizer** needs (not the hot `onFinish` path) must therefore also be written to a
    durable sidecar file under `.gradle/buildhound/` (survives `clean`, lifecycle tracks the CC
    entry), read by the finalizer with the service param as fallback. First consumer:
    `TestLocationSidecar` for Test-task JUnit XML dirs (plan 024). The `type`/`cacheable`
    dictionary is read on `onFinish` and stays on the frozen-param path — its composite-build gap
    is tracked in plan 045.
