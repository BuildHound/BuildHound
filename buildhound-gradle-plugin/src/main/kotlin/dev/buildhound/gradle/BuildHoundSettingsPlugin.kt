...
abstract class BuildHoundSettingsPlugin
@Inject
constructor(
    ...
    // Gradle registration is deliberately centralized so configuration-cache ownership and lifecycle
    // hooks remain visible in one place.
    @Suppress("CyclomaticComplexMethod", "LongMethod")
    override fun apply(settings: Settings) {
        val extension = settings.extensions.create("buildhound", BuildHoundExtension::class.java)
        // Config overrides (plan 027): apply `buildhound.<key>`/`BUILDHOUND_<KEY>` as the
        // convention
        // fallback so precedence is explicit-DSL > override > default. server.token is NOT
        // overridable.
        ...
        extension.localBuilds.requireOptInFile.convention(
            overrides.bool("localBuilds.requireOptInFile") ?: true
        )
        extension.upload.uploadInBackground.convention(
            overrides.bool("upload.uploadInBackground") ?: false
        )
        ...
        // default. Read at whenReady (post-DSL) and handed to the module's wiring; all-off touches
        // no
        // internal API. Standard override precedence (like
        // htmlReport.enabled/processProbe.enabled):
        // explicit DSL value > buildhound.internalAdapters.* override > false.
        extension.internalAdapters.collectCacheOrigins.convention(
            overrides.bool("internalAdapters.collectCacheOrigins") ?: false
        )
        extension.internalAdapters.collectDeprecations.convention(
            overrides.bool("internalAdapters.collectDeprecations") ?: false
        )
        extension.internalAdapters.collectLogWarnings.convention(
            overrides.bool("internalAdapters.collectLogWarnings") ?: false
        )
        extension.internalAdapters.perFileHashes.convention(
            overrides.bool("internalAdapters.perFileHashes") ?: false
        )
        ...
        if (settings.gradle.parent != null) {
            logger.info(
                "[buildhound] applied in an included build; the root build's plugin collects"
            )
            return
        }
        ...
        // Sibling mailbox for the JVM archive-task output locations (plan 072, research F22),
        // filled by
        // the same `whenReady` callback (from graph.allTasks — only tasks scheduled this build) and
        // replayed from the CC entry on a hit via the finalizer's jvmArtifacts param below.
        // Locations
        // only — the File.length() read happens at Flow time (no config-phase file read → no CC
        // input).
        ...
        // {}`
        // block is guaranteed fully evaluated (never at apply(), where it may be empty). Reading
        // only
        // `settings.buildCache` here touches no `Gradle.includedBuilds` (the call that forced the
        // other
        // walks off `settingsEvaluated`), so this hook is safe. Replayed from the CC entry on a hit
        // via
        ...
        settings.gradle.settingsEvaluated { s ->
            runCatching { buildCacheHolder.set(BuildCacheConfigReader.snapshot(s.buildCache)) }
                .onFailure {
                    logger.warn(
                        "[buildhound] build-cache config snapshot failed (build unaffected): {}",
                        it.message,
                    )
                }
        }
        // Sibling mailbox for the change blast-radius module-dir index (plan 063, research F13):
        // the
        // Gradle `path → projectDir-relative-to-root` map, filled at `settingsEvaluated` — the
        // earliest
        // hook where the descriptor tree is populated (post-`include()`) and reading it touches no
        // `Gradle.includedBuilds` (the call that forced plan 069's walk off `settingsEvaluated`).
        // Baked
        // into the ChangedModulesValueSource params below (same channel as buildStructureHolder)
        // and
        // replayed on a CC hit — correct, since module structure is CC-keyed. Its own guard so a
        // walk
        // failure degrades the index to empty (→ honest `unattributedChanges`), never a failed
        // build.
        ...
        settings.gradle.settingsEvaluated { s ->
            runCatching { moduleDirIndexHolder.set(ModuleDirIndexWalker.walk(s)) }
                .onFailure {
                    logger.warn(
                        "[buildhound] module-dir index walk failed (build unaffected): {}",
                        it.message,
                    )
                }
        }
        ...
        val toolchainSeam =
            DetectedToolchain(
                ...
                springBoot =
                    settings.providers
                        .gradleProperty("buildhound.internal.toolchain.springBoot")
                        .orNull,
            )
        ...
        val collector =
            settings.gradle.sharedServices.registerIfAbsent(
                ...
                spec.parameters.testResultLocations.set(
                    settings.providers.provider { testLocationsHolder.get() }
                )
                // Start-marker context (plan 033): only CC-stable fields (no ci/vcs value source —
                // a
                // service param bakes and replays stale on a hit). Null when telemetry is off → no
                // marker.
                // Mode is resolved with no CI context (an AUTO build's marker is LOCAL); the
                // connector
                // fallback upgrades genuine CI cases. requestedTasks is part of the CC key, so it
                // is never
                ...
                spec.parameters.markerContext.set(
                    settings.providers.provider {
                        ...
                        MarkerContext(
                            startedDir =
                                File(settings.rootDir, "build/buildhound/started").absolutePath,
        ...
        val failBuildStructureSnapshot =
            settings.providers
        ...
        settings.gradle.taskGraph.whenReady { graph ->
            ...
            runCatching {
                ...
                if (isolatedProjects) {
                    logger.info(
                        "[buildhound] isolated projects active; task metadata dictionary left empty"
                    )
                } else {
                    taskMetadataHolder.set(
                        graph.allTasks.associate { task ->
                            task.path to TaskClassIntrospection.introspect(task.javaClass)
                        }
                    )
                    // Test-result locations (plan 024): capture each Test task's JUnit XML dir now,
                    // at
                    // config time (spike §4a) — the XML itself is read in the finalizer. Public
                    // Test API
                    // only; the decision to treat a task as a Test task is the type-free
                    // introspection.
                    if (collectTests.getOrElse(true)) {
                        ...
                        // instantiated by included-build task events *before* this callback runs,
                        // freezing
                        // its params empty — so the finalizer reads these locations from the
                        // sidecar file
                        // instead. Under .gradle (like the salt) so it survives `clean` and tracks
                        // the CC
                        // entry. The holder/param above stays as the classpath-path fallback. The
                        // write
                        // respects the master switch (spec §3.4) — a disabled build must touch
                        // nothing on
                        // disk, parity with the salt/start-marker.
                        if (
                            extension.enabled.get() &&
                                extension.mode.get() != TelemetryMode.DISABLED
                        ) {
                ...
                .onFailure {
                    logger.warn(
                        "[buildhound] task metadata capture failed (build unaffected): {}",
                        it.message,
                    )
                }
            // Toolchain versions (plan 046): its own guard, after the higher-stakes task/test
            // capture,
            // so a detection failure can never block them. The seam wins for tests; otherwise walk
            // every
            ...
            // cross-project walk is illegal and the dimensions degrade to null like the task
            // dictionary.
            runCatching {
                toolchainHolder.set(
                    when {
                ...
                .onFailure {
                    logger.warn(
                        "[buildhound] toolchain detection failed (build unaffected): {}",
                        it.message,
                    )
                }
            // JVM archive-size locations (plan 072, research F22): its own guard, after the
            // higher-stakes
            // task/test capture, so a failure can never block them. Filter graph.allTasks (only
            // tasks
            // scheduled this build = "what's built") to core-Gradle AbstractArchiveTask whose name
            // is a
            // known JVM archive; record the output *location* only (no file read → no CC input).
            // Skipped
            // under isolated projects, where the graph walk is illegal — degrades to empty like the
            // task
            // dictionary and toolchain triple. AbstractArchiveTask is public core API always on the
            // plugin
            // classpath, so — unlike the plan-031 AGP path — there is no external classloader to
            // fault.
            runCatching {
                ...
                .onFailure {
                    logger.warn(
                        "[buildhound] jvm artifact-location capture failed (build unaffected): {}",
                        it.message,
                    )
                }
            // Internal-adapters capture (plan 074): driven from here, post-DSL, so the toggles are
            // set.
            // The wiring is fully guarded and no-ops when every effective toggle is off — the point
            // where
            // "applied the plugin" stops short of "consented to internal Gradle APIs". Registering
            // the
            // daemon listeners here (not at apply) mirrors the WARN-log listener's original site;
            // on a CC
            // hit whenReady is skipped and capture rides the daemon-static listener from the first
            // miss.
            //
            // The master switch gates every toggle: with telemetry off (enabled=false or
            // mode=DISABLED)
            // the finalizer short-circuits before the read-and-clear collector runs, so a capturing
            // build
            // here would leave stale rows in the daemon-static accumulator that a later on-build
            // would
            // emit. Forcing the toggles false when off means the wiring still calls configure()
            // (resetting
            // the daemon-static toggles, so a lingering listener from an earlier on-build gates to
            // no-op)
            // but registers nothing and captures nothing — parity with the other master-gated
            // collectors.
            val telemetryOn =
                extension.enabled.get() && extension.mode.get() != TelemetryMode.DISABLED
            runCatching {
                InternalAdaptersWiring.install(
                    settings = settings,
                    graph = graph,
                    collectCacheOrigins =
                        telemetryOn && extension.internalAdapters.collectCacheOrigins.get(),
                    collectDeprecations =
                        telemetryOn && extension.internalAdapters.collectDeprecations.get(),
                    collectLogWarnings =
                        telemetryOn && extension.internalAdapters.collectLogWarnings.get(),
                    perFileHashes = telemetryOn && extension.internalAdapters.perFileHashes.get(),
                )
            }
                .onFailure {
                    logger.warn(
                        "[buildhound] internal-adapters wiring failed (build unaffected): {}",
                        it.message,
                    )
                }
        }
        ...
        val environment =
            settings.providers.of(EnvironmentValueSource::class.java) { spec ->
        ...
        val vcs =
            settings.providers.of(VcsValueSource::class.java) { spec ->
                spec.parameters.enabled.set(extension.enabled)
                ...
                spec.parameters.timeoutMillis.set(
                    settings.providers.gradleProperty("buildhound.vcs.timeout.ms").map { raw ->
                        raw.toLongOrNull()?.takeIf { it > 0 } ?: GitExec.DEFAULT_TIMEOUT_MS
                    }
                )
                // Discover the enclosing repo from a nested Gradle root (plan 050). Only an
                // explicit
                // "false" confines the probes to rootDir (pre-050, fail-closed); absent or an
                // unrecognized value keeps the default-on discovery via the ValueSource's
                // getOrElse(true),
                // so a typo never silently disables it (fail toward the default, not toward
                // confined).
                spec.parameters.searchParents.set(
                    settings.providers.gradleProperty("buildhound.vcs.searchParents").map {
                        !it.trim().equals("false", ignoreCase = true)
                    }
                )
            }

        val ci =
            settings.providers.of(CiValueSource::class.java) { spec ->
                spec.parameters.enabled.set(extension.enabled)
            }

        // Change blast-radius attribution (plan 063, research F13): the module-dir index is baked
        // from
        // the `settingsEvaluated` walk above (CC-keyed module structure); every git call happens
        // inside
        // obtain() at execution time (the VcsValueSource CC rationale) so nothing here becomes a CC
        // fingerprint input and the diff stays fresh across a CC hit. targetBranch is wired from
        // the CI
        // context's PR base ref (execution-time value source), the vcs.timeout/searchParents knobs
        // are
        // shared with VcsValueSource, and lastShaPath is the previous-build HEAD file the finalizer
        // writes.
        val changedModules =
            settings.providers.of(ChangedModulesValueSource::class.java) { spec ->
                spec.parameters.enabled.set(extension.enabled)
                spec.parameters.rootDir.set(settings.rootDir.absolutePath)
                spec.parameters.timeoutMillis.set(
                    settings.providers.gradleProperty("buildhound.vcs.timeout.ms").map { raw ->
                ...
                spec.parameters.lastShaPath.set(
                    File(settings.rootDir, LAST_BUILT_SHA_PATH).absolutePath
                )
                // CI PR base ref (plan 041): a merge-base diff scoped to the PR's own changes. An
                // absent CI
                // context, or a CI context with no target branch, both leave this unset (`.map` on
                // an absent
                ...
                spec.parameters.moduleDirIndex.set(
                    settings.providers.provider { moduleDirIndexHolder.get() }
                )
            }

        // Benchmark activation (plan 030): reads BUILDHOUND_BENCHMARK_* at execution time (no CC
        // input),
        // same discipline as CiValueSource. A present, valid scenario forces mode=benchmark.
        val benchmark =
            settings.providers.of(BenchmarkValueSource::class.java) { spec ->
                spec.parameters.enabled.set(extension.enabled)
            }

        // Android artifact-size collector (plan 031): a per-project reaction wired from the
        // settings
        // context. beforeProject is the isolated-projects-safe hook; the collector class-references
        // no
        ...
        // measuring tasks run at execution time; the Flow finalizer reads their output at build
        // end.
        if (masterEnabled) {
            installAndroidArtifactCollector(
                settings.gradle,
                File(settings.rootDir, "build/buildhound/artifacts"),
            )
            // Per-project configuration-time attribution (plan 052): a per-project reaction wired
            // the
            // same way — beforeProject/afterProject are themselves the isolated-projects-safe
            // hooks,
            // deliberately NOT further gated on isolated projects (there is no cross-project state
            // here
            // to degrade). Under `.gradle` (not `build/`) so a same-invocation `clean` can't wipe
            // it (the
            // plan-044 rationale, same as the identity salt / test-location sidecar).
            installProjectEvaluationCollector(
                settings.gradle,
                File(settings.rootDir, ".gradle/buildhound/config-timings"),
            )
            ...
            // read (settingsEvaluated throws on Gradle.includedBuilds, verified empirically) —
            // still
            ...
            settings.gradle.projectsLoaded {
                runCatching {
                    ...
                    .onFailure {
                        logger.warn(
                            "[buildhound] build-structure descriptor walk failed (build unaffected): {}",
                            it.message,
                        )
                    }
            }
        ...
        // End-of-build JVM process probe (plan 029). enabled is master AND the block toggle; the
        // exec
        ...
        val processes =
            settings.providers.of(ProcessProbeValueSource::class.java) { spec ->
                spec.parameters.enabled.set(
                    extension.enabled.zip(extension.processProbe.enabled) { master, probe ->
                        master && probe
                    }
                )
                spec.parameters.timeoutMillis.set(
                    settings.providers.gradleProperty("buildhound.processprobe.timeout.ms").map {
                        raw ->
                        raw.toLongOrNull()?.takeIf { it > 0 } ?: GitExec.DEFAULT_TIMEOUT_MS
                    }
                )
        ...
        val fingerprints =
            settings.providers.of(FingerprintValueSource::class.java) { spec ->
                spec.parameters.enabled.set(extension.enabled)
                spec.parameters.identitySaltFile.set(saltPath)
                ...
                spec.parameters.gradleProperties.set(
                    // An unset property (or one set to "") is treated as absent — no
                    // gradleProp-<name>
                    ...
                    extension.fingerprints.gradleProperties.map { names ->
                        names
                            .associateWith { name ->
                                settings.providers.gradleProperty(name).getOrElse("")
                            }
                            .filterValues { it.isNotEmpty() }
                    }
                )
                spec.parameters.parallel.set(
                    settings.startParameter.isParallelProjectExecutionEnabled
                )
                spec.parameters.maxWorkers.set(settings.startParameter.maxWorkerCount)
            }

        // Invocation-switch & performance-flag posture (plan 051). The seven StartParameter
        // scalars,
        // plus the -P project-properties map (also only reachable at apply()-time), are baked
        // config-time params (same narrowing as the fingerprints parallel/maxWorkers above); only
        // the
        ...
        val invocation =
            settings.providers.of(InvocationValueSource::class.java) { spec ->
                spec.parameters.enabled.set(extension.enabled)
                ...
                spec.parameters.refreshDependencies.set(
                    settings.startParameter.isRefreshDependencies
                )
                ...
                spec.parameters.projectPropertiesPath.set(
                    File(settings.rootDir, "gradle.properties").absolutePath
                )
                spec.parameters.gradleUserHomePropertiesPath.set(
                    File(settings.startParameter.gradleUserHomeDir, "gradle.properties")
                        .absolutePath
                )
                // android.* keys are AGP-read project properties, not Gradle properties: their real
                // command-line override channel is -P (StartParameter.projectProperties), not -D.
                // Baked
                // here alongside the scalars above (StartParameter is only reachable at
                // apply()-time);
                // GradlePropertyProvenance picks this vs. the -D sysprop channel per key family
                // (051 review).
        ...
        // the descriptor map + counts into these ValueSource parameters at configuration time;
        // obtain()
        // runs the filesystem .exists() probes at execution time
        // (VcsValueSource/FingerprintValueSource
        ...
        val buildStructure =
            settings.providers.of(BuildStructureValueSource::class.java) { spec ->
                spec.parameters.enabled.set(extension.enabled)
                spec.parameters.rootDir.set(settings.rootDir.absolutePath)
                spec.parameters.projectCount.set(
                    settings.providers.provider { buildStructureHolder.get().projectCount }
                )
                spec.parameters.maxDepth.set(
                    settings.providers.provider { buildStructureHolder.get().maxDepth }
                )
                spec.parameters.includedBuildCount.set(
                    settings.providers.provider { buildStructureHolder.get().includedBuildCount }
                )
                spec.parameters.descriptors.set(
                    settings.providers.provider { buildStructureHolder.get().descriptors }
                )
            }
        ...
        val wrapper =
            settings.providers.of(WrapperValueSource::class.java) { spec ->
                spec.parameters.enabled.set(extension.enabled)
                spec.parameters.propertiesFile.set(
                    File(settings.rootDir, "gradle/wrapper/gradle-wrapper.properties").absolutePath
                )
                spec.parameters.jarFile.set(
                    File(settings.rootDir, "gradle/wrapper/gradle-wrapper.jar").absolutePath
                )
                spec.parameters.gradleUserHomeDir.set(
                    settings.startParameter.gradleUserHomeDir.absolutePath
                )
            }
        ...
        flowScope.always(TelemetryFinalizerAction::class.java) { spec ->
            ...
            // AGP/KGP/KSP versions (plan 046): a finalizer parameter (not a collector service
            // param) so
            // the provider resolves after configuration — after `whenReady` fills the mailbox —
            // even in
            // a composite build where an included build's task instantiates the collector early.
            // The
            ...
            // Flow-action channel as [toolchain] — resolved after `whenReady` fills the mailbox,
            // baked
            // into the CC entry, replayed on a hit. The finalizer reads File.length() at execution
            // time.
            spec.parameters.jvmArtifacts.set(
                settings.providers.provider { jvmArtifactsHolder.get() }
            )
            ...
            spec.parameters.taskMetadata.set(
                settings.providers.provider { taskMetadataHolder.get() }
            )
            ...
            // Failure detail (plan 044): extract class/message/stacktrace from the failing
            // Throwable
            // inside the provider map — runs at finalization (execution time), so it stays CC-safe
            // and
            ...
            // Change blast-radius attribution (plan 063): the ValueSource re-obtains on a CC hit
            // like
            // vcs (same replay caveat) — an absent (no base resolvable) value leaves the block off
            // the payload.
            ...
            // time — parallel to configurationCacheRequested below — so it stays accurate whether
            // the
            // finalizer runs after a CC store or replays a CC hit.
            spec.parameters.isolatedProjectsActive.set(
            ...
            // Flow-action channel as [toolchain] — the provider resolves after `settingsEvaluated`
            // fills
            // the mailbox, is baked into the CC entry, and replays on a hit (correct: cache config
            // is
            // stable across builds). Public Settings.buildCache read only; no URL/path ever
            // captured.
            spec.parameters.buildCache.set(settings.providers.provider { buildCacheHolder.get() })
            spec.parameters.configurationCacheRequested.set(
                buildFeatures.configurationCache.requested.getOrElse(false)
            )
            // org.gradle.configuration-cache.parallel flag (plan 064, research F14): a provider
            // read of
            // the Gradle property — a tracked CC input, resolved after config and replayed on a hit
            // —
            // never System.getProperty. BuildFeatures surfaces only .requested/.active, so the flag
            // has
            // to come from the property (plan-064 divergence note). Unset → the Optional param
            // stays absent.
            spec.parameters.configurationCacheParallel.set(
                settings.providers.gradleProperty("org.gradle.configuration-cache.parallel").map {
                    it.toBoolean()
                }
            )
            // Lazy: the settings script sets rootProject.name after apply() runs.
            spec.parameters.projectKey.set(
                settings.providers.provider { settings.rootProject.name }
            )
            spec.parameters.requestedTasks.set(settings.startParameter.taskNames.toList())
            // Excluded task names (plan 054): the public CC-safe StartParameter.excludedTaskNames
            // (a Set),
            // sorted for determinism — part of the CC key like requestedTasks, replayed verbatim on
            // a hit.
            spec.parameters.excludedTaskNames.set(
                settings.startParameter.excludedTaskNames.sorted()
            )
            ...
            spec.parameters.kotlinReportOutput.set(
                settings.providers.gradleProperty("kotlin.build.report.output")
            )
            spec.parameters.kotlinJsonDirectory.set(
                settings.providers.gradleProperty("kotlin.build.report.json.directory")
            )
            ...
            spec.parameters.failTestCollection.set(
                settings.providers
                    .gradleProperty("buildhound.internal.failTestCollection")
                    .map { it != "false" }
                    .orElse(false)
            )
            spec.parameters.rootDir.set(settings.rootDir.absolutePath)
            ...
            // uploadInBackground (plan 027): a local build spools instead of attempting the inline
            // upload.
            ...
            spec.parameters.optInFile.set(
                settings.providers.gradleProperty("buildhound.optin.file")
            )
        }
    }
     ...
     * type. It must run here, in `whenReady`, never in `apply()`: `required` (like
     * `outputLocation`) only reflects the task's real configuration after `afterEvaluate`, which
     * `whenReady` guarantees and `apply()` does not.
     */
    private fun testLocationOf(task: Task): Pair<String, TestResultLocations>? {
        ...
        return task.path to
            TestResultLocations(
                junitXmlDir = dir,
                module = module,
                junitXmlRequired = junitXmlRequired,
            )
    }

    /**
     * The output location + kind for one core-Gradle JVM archive task (plan 072, research F22), or
     * null when [task] is not an [AbstractArchiveTask] whose name is a known JVM archive
     * (`bootJar`/`bootWar`/`jar`/`war`). Runs at configuration time inside `whenReady`, so
     * resolving the archive-file *location* is CC-safe (a resolved path, not a file read — mirrors
     * [testLocationOf]). The `bootJar`/`bootWar` names cover the Spring Boot deliverables;
     * `jar`/`war` the plain archives.
     */
    ...
    private companion object {
        ...
        /**
         * Previous-build HEAD sha, the LAST_BUILT_SHA diff base (plan 063); under `.gradle` so it
         * survives `clean`.
         */
        ...
        /**
         * Canonical JVM archive task names → [JvmArtifactKind] (plan 072). The Spring Boot
         * deliverables (`bootJar`/`bootWar`) plus the plain Java/War archives; a task must ALSO be
         * an [AbstractArchiveTask] to match, so an unrelated task that happens to share one of
         * these names is not captured.
         */
        val JVM_ARCHIVE_KINDS: Map<String, JvmArtifactKind> =
            mapOf(
 ...
 */
@Suppress("UnstableApiUsage")
private fun installAndroidArtifactCollector(
    ...
    gradle.lifecycle.beforeProject { project ->
        runCatching { AndroidArtifactCollector.install(project, artifactsDir) }
            .onFailure {
                project.logger.info(
                    "[buildhound] android artifact collector unavailable: {}",
 ...
 * marks the start via a project-scoped extra property — the only state the two `IsolatedAction`s
 * can share, since each runs isolated and cannot see a plugin-captured mutable holder (narrowing 2)
 ...
 */
@Suppress("UnstableApiUsage")
private fun installProjectEvaluationCollector(
    gradle: org.gradle.api.invocation.Gradle,
    timingsDir: File,
) {
    gradle.lifecycle.beforeProject { project ->
        runCatching {
                project.extensions.extraProperties.set(PROJECT_EVAL_START_KEY, System.nanoTime())
            }
            .onFailure {
                project.logger.info(
                    "[buildhound] project-evaluation timing start unavailable: {}",
                    it::class.java.simpleName,
                )
            }
    }
    gradle.lifecycle.afterProject { project ->
        runCatching {
            ...
            .onFailure {
                project.logger.info(
                    "[buildhound] project-evaluation timing capture unavailable: {}",
                    it::class.java.simpleName,
                )
            }
    }
