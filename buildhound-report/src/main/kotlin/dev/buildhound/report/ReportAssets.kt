...
object ReportAssets {
    ...
    /**
     * The template with the shared [timelineJs] renderer spliced into its script element,
     * so `template()` always returns the complete self-contained document — the existing
     * zero-network test then covers the timeline code for free (plan 017). The
     * [DATA_PLACEHOLDER] is left intact for [render].
     */
    fun template(): String {
        val spliced = rawTemplate().replace(TIMELINE_PLACEHOLDER, timelineJs())
        check(!spliced.contains(TIMELINE_PLACEHOLDER)) { "timeline placeholder was not spliced into the report template" }
        return spliced
    }
    ...
    fun timelineJs(): String =
        resource("/dev/buildhound/report/timeline.js").also {
            ...
            require(!it.contains("</script", ignoreCase = true)) { "timeline.js must not contain a </script sequence" }
        }
