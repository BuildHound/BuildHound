...
| Asset | Purpose |
|---|---|
| `azure-pipelines/btp-gradle-steps.yml` | Reusable Azure Pipelines steps template: token injection, `gradle.properties` validation, HTML-artifact publishing, optional verdict gate |
| `bin/btp-metric` | Metric CLI for non-Gradle steps → `POST /v1/metrics` (Datadog tag/measure model) |
